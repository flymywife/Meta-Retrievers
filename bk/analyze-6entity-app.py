...
def analyze_json_files(files, model_name):
    ...
    for file in files:
        ...
        entities = data.get('entities', ['Unknown'])
        error_count = 0
    ...
    html_output = "<table border='1'><tr><th>Entities</th><th>Max Tokens</th><th>Temperature</th><th>Error Count</th></tr>"
    for result in results:
        html_output += f"<tr><td>{result['Entities']}</td><td>{result['Max Tokens']}</td><td>{result['Temperature']}</td><td>{'No errors' if result['Error Count'] == 0 else result['Error Count']}</td></tr>"
    html_output += "</table>"

    # Generate output filename
    current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
    entities_for_filename = '_'.join(results[0]['Entities'].split(', ')[:6])  # 最初の6エンティティを使用
    output_filename = f"{model_name}_{entities_for_filename}_{current_time}.html"
